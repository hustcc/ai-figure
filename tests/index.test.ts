...
// ---------------------------------------------------------------------------
// fig(string) — unified markdown API + fault tolerance (streaming)
// ---------------------------------------------------------------------------
...
describe('fig(string) — markdown input', () => {
  ...
  // Fault-tolerance / streaming safety
  it('returns empty SVG for an empty string (never throws)', () => {
    const svg = fig('');
    expect(svg).toContain('<svg');
    expect(() => fig('')).not.toThrow();
  });

  it('returns empty SVG for a blank-only string (never throws)', () => {
    const svg = fig('   \n  \n');
    expect(svg).toContain('<svg');
  });
  ...
  it('handles partial/streaming input — header + partial edge line', () => {
    // Incomplete edge "a -->" should be ignored, not crash
    const svg = fig('figure flow\na -->');
    expect(svg).toContain('<svg');
  });
  ...
  it('renders progressively — more content = richer diagram', () => {
    ...
    // Full diagram should contain the extra node label
